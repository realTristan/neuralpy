...
if __name__ == "__main__":
    for i in range(1):
        # Download drunk images
        url: str = drunk_url(i + 1)
        images: bs4.element.ResultSet = get_images(url)
        download_images(images, "datasets/drunk_sober/images/drunk/")
        print(f"Downloaded {len(images)} images from {url}")

        # Download sober images
        url: str = sober_url(i)
        images: bs4.element.ResultSet = get_images(url)
        download_images(images, "datasets/drunk_sober/images/sober/")
        print(f"Downloaded {len(images)} images from {url}")
